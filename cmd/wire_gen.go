...
func NewListOrderUseCase(db *sql.DB, eventDispatcher events.EventDispatcherInterface) *usecase.ListOrderUseCase {
	orderRepository := database.NewOrderRepository(db)
	listOrder := event.NewListOrder()
	listOrderUseCase := usecase.NewListOrderUseCase(orderRepository, listOrder, eventDispatcher)
...
var setListOrderdEvent = wire.NewSet(event.NewListOrder, wire.Bind(new(events.EventInterface), new(*event.ListOrder)))
